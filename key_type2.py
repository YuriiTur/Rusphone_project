...
class KeyboardAnalyzer:
    ...
    def _init_rusphone_layout(self):
        """Фонетическая русская раскладка (Q → Я)"""
        self.keys = {
            'я': (16, 'left_pinky'), 'в': (17, 'left_ring'),
            'е': (18, 'left_middle'), 'р': (19, 'left_index'),
            'т': (20, 'left_index'), 'ы': (21, 'right_index'),
            'у': (22, 'right_index'), 'и': (23, 'right_middle'),
            'о': (24, 'right_ring'), 'п': (25, 'right_pinky'),
            'а': (30, 'left_pinky'), 'с': (31, 'left_ring'),
            'д': (32, 'left_middle'), 'ф': (33, 'left_index'),
            'г': (34, 'left_index'), 'х': (35, 'right_index'),
            'й': (36, 'right_index'), 'к': (37, 'right_middle'),
            'л': (38, 'right_ring'), 'ж': (39, 'right_pinky'),
            'з': (44, 'left_pinky'), 'ь': (45, 'left_ring'),
            'ц': (46, 'left_middle'), 'ж': (47, 'left_index'),
            'б': (48, 'left_index'), 'н': (49, 'right_index'),
            'м': (50, 'right_index'), ',': (51, 'right_middle'),
            '.': (52, 'right_ring'), ' ': (57, 'right_thumb')
        }

        self.shift_keys = {
            '!': (2, 'left_pinky'), '"': (3, 'left_ring'),
            '№': (4, 'left_middle'), ';': (5, 'left_index'),
            '%': (6, 'right_index'), ':': (7, 'right_middle'),
            '?': (8, 'right_ring'), '*': (9, 'right_pinky'),
            ',': (53, 'right_pinky')
        }

        self.home_positions = {
            'left_pinky': 30, 'left_ring': 31, 'left_middle': 32,
            'left_index': 33, 'right_index': 36, 'right_middle': 37,
            'right_ring': 38, 'right_pinky': 39,
            'left_thumb': 42, 'right_thumb': 57
        }           
...
for i in range(3):  # для трёх текстов
    for layout_code, layout_name in layouts:
        ...
        if i < len(results):
            ...
            layout_display_name = (
                'ЙЦУКЕН' if layout_code == 'standard'
                else 'Вызов' if layout_code == 'challenge'
                else 'Русфон'
            )

            print(f"{result['text_name']:<15} {layout_display_name:<12} "
