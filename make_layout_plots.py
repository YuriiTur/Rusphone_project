...
def parse_summary_table(lines):
    ...
    i += 1
    # пропускаем линию из дефисов, если есть
    ...
    while i < n:
        ...
        if stripped.startswith("================================================================================") or stripped.startswith("ДЕТАЛЬНЫЙ"):
            break
        # возможная линия из дефисов
        if set(stripped) <= {"-"}:
            i += 1
            continue

        tokens = re.split(r"\s+", stripped)
        # ожидаем не меньше 20 токенов
        if len(tokens) < 20:
            i += 1
            continue
        try:
            layout = tokens[0]
            symbols = int(tokens[1].replace(" ", ""))
            presses = int(tokens[2].replace(" ", ""))
            n_per_sym = float(tokens[3].replace(",", "."))
            total_path = int(tokens[4].replace(" ", ""))
            avg_path = float(tokens[5].replace(",", "."))
            left_pct = float(tokens[6].replace(",", "."))
            right_pct = float(tokens[8].replace(",", "."))
            twohands_pct = float(tokens[10].replace(",", "."))
            shift_count = int(tokens[12].replace(" ", ""))
            alt_count = int(tokens[13].replace(" ", ""))
            bigram_ud_pct = float(tokens[14].rstrip("%").replace(",", "."))
            bigram_chud_pct = float(tokens[15].rstrip("%").replace(",", "."))
            bigram_nud_pct = float(tokens[16].rstrip("%").replace(",", "."))
            trigram_ud_pct = float(tokens[17].rstrip("%").replace(",", "."))
            trigram_chud_pct = float(tokens[18].rstrip("%").replace(",", "."))
            trigram_nud_pct = float(tokens[19].rstrip("%").replace(",", "."))
        except Exception:
            i += 1
            continue
...
def parse_analysis_file(path):
    ...
    while i < n:
        ...
        if m:
            ...
            # подливаем в эту раскладку данные из сводной таблицы, если они есть
...
def make_plots_for_file(path, out_root="plots"):
    ...
    plot_bar(layout_names, total_path,
    ...
    plot_bar(layout_names, avg_path,
             f"Средний путь (текст: {text_name})",
             "Условные единицы пути",
             os.path.join(out_dir, "avg_path.png"))

    # 4. Руки
    plot_stacked_hands(layout_names, layouts,
    ...
    if any(layouts[name]["fingers"] for name in layout_names):
        plot_finger_stack(layout_names, layouts,
    ...
    if any(layouts[name].get("shift_count") is not None for name in layout_names):
        plot_shift_alt(layout_names, layouts,
    ...
    if any(layouts[name].get("bigram_ud_pct") is not None for name in layout_names):
        plot_ngrams_group(layout_names, layouts,
